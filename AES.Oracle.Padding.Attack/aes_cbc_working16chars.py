...
#Credit to CorvusCrypto from Youtube
#This attack is elegantly preparing the padding numbers for guessing
...
def find_last_byte (ciphertext):
        ...
        #Copy of block[0] whic is the IV
        ...
        for i in range (16):
            #Last bit to first bit Increments by 1 
            #!!!!!This function prepares the padding to change the bit to be changed accordingly
            #That is, <guess>x03x03  /   <guess>x02  
            expected_padding= bytearray([0 for _ in range(16-i)] + [(i+1) for _ in range(i)])

            #XOR if all O's (plaintext_bytes) to any number returns that number
            #This means c_prime is the same value as expected_padding
        ...
        #This means to cover from item 0 to 255 characters (or 256 characters)
            for byte in itertools.chain (range(blocks[0][15-1] + 1, 256),range (0, blocks[0] [15-i] + 1)):
                ...
                c_prime[15-i] = byte
                print (c_prime)
                print(f"c_primte[15-i]:{c_prime[15-i]}")
